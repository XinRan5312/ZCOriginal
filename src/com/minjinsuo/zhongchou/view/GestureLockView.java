...
public class GestureLockView extends View {
	...
	/** 未选中颜色 */
	private final int NORMAL_COLOR = Color.parseColor("#cccccc");
	/** 大圆圈的颜色 */
	private final int CIRCLEL_COLOR = Color.parseColor("#cccccc");

	/** 错误颜色 */
	private final int ERROE_COLOR = Color.parseColor("#FF2525"); // 正常外圆颜色

	/** 选中时颜色 */
	// private final int TOUCH_COLOR = Color.parseColor("#009900"); // 选中内圆颜色
	private final int TOUCH_COLOR = getResources().getColor(R.color.main_color); // 选中内圆颜色
	// private final int TOUCH_COLOR =
	// getResources().getColor(R.color.gesture_draw); // 选中内圆颜色
	private final int COVER_COLOR = getResources().getColor(R.color.bg); // 覆盖中心圆颜色
		...
		if (cycles == null && (perWidthSize > 0) && (perHeightSize > 0)) {
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					...
					lockCircle.setR(perWidthSize * 0.6125f);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < cycleSize; i++) {
			drawOutsideCycleCover(cycles[i], canvas, COVER_COLOR);
			// 画完并且错误
			if (!canContinue && !result) {
			...
			// 绘画中
				...
				if (cycles[i].isOnTouch()) {
					...
					drawOutsideCycle(cycles[i], canvas, CIRCLEL_COLOR);
				} else
					drawOutsideCycle(cycles[i], canvas, NORMAL_COLOR);
			}
		}
	...
	/** 画覆盖中心圆 */
	private void drawOutsideCycleCover(LockCircle lockCircle, Canvas canvas,
			int color) {
		paintCover.setColor(color);
		canvas.drawCircle(lockCircle.getOx(), lockCircle.getOy(),
				lockCircle.getR(), paintCover);
	}
