...
public class ObservableScrollView extends ScrollView {
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (imageView.isShown() && imageView.getTop() >= 0) {
				if (tool != null) {
					...
					if (!scrollerType && currentY < lastLy
							&& imageView.getHeight() > imageViewH) {
						scrollTo(0, 0);
						imageView.getLocationInWindow(li);
						getLocationOnScreen(li2);
						...
						if (imageView.getHeight() == imageViewH
								&& li[1] == li2[1]) {
							scrollerType = true;
						}
						if (startIsTop && li[1] != li2[1]) {
		...
		case MotionEvent.ACTION_UP:
			if (li[1] == li2[1]) {
				scrollerType = true;
				mScroller.startScroll(imageView.getLeft(),
						imageView.getBottom(), 0 - imageView.getLeft(),
