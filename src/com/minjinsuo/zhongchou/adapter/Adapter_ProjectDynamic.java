...
import net.zkbc.p2p.fep.message.protocol.GetPrjProgressListResponse.ElementPrjProgressList;
...
/**
 * 项目动态进展展示；编辑项目动态 （复用） 适配器
 */
public class Adapter_ProjectDynamic extends BaseAdapter {

	public Context context;
	private ArrayList<ElementPrjProgressList> tenderList;
	private ImageOptions options;

	public Adapter_ProjectDynamic(Context context) {
		super();
		this.context = context;
		// 初始化
		tenderList = new ArrayList<ElementPrjProgressList>();
	...
	public void setData(List<ElementPrjProgressList> tenderList) {
		if (tenderList != null) {
	...
	public ArrayList<ElementPrjProgressList> getData() {
		return tenderList;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.tv_title_dynamic = (TextView) convertView
					.findViewById(R.id.tv_title_dynamic);// 标题
		...
		ElementPrjProgressList info = tenderList.get(position);
		if (info != null) {
			// 处理进度线样式
			if (position == 0) {
				...
				holder.tv_content.setTextColor(context.getResources().getColor(
						R.color.green));
				holder.tv_title_dynamic.setTextColor(context.getResources()
	...
	public static class ViewHolder {
		public TextView tv_time, tv_content, tv_title_dynamic;
