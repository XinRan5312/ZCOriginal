 ...
 * 新增/修改 收货地址
 */
public class Activity_ReceiverAddress extends Activity_Base implements
		OnClickListener {
	...
	protected void initView() {
		...
		ll_choseCity.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(Activity_ReceiverAddress.this,
						Activity_city.class);
	...
	public void onClick(View v) {
		...
		case R.id.btnNext:
			if (StringUtils.isEmpty(tvEar01.getText().toString())) {
				ToastUtil.showToast(Activity_ReceiverAddress.this, "所在地区不能为空");
				return;
			}
			if (StringUtils.isEmpty(etNowAddress.getText().toString())) {
				ToastUtil.showToast(Activity_ReceiverAddress.this, "详细地址不能为空");
				return;
			}
			if (StringUtils.isEmpty(et_receiver.getText().toString())) {
				ToastUtil.showToast(Activity_ReceiverAddress.this, "收货人不能为空");
				return;
			}
			if (StringUtils.isEmpty(et_phoneNumber.getText().toString())) {
				ToastUtil.showToast(Activity_ReceiverAddress.this, "手机号不能为空");
				return;
			}

			if (!StringUtils.isPhoneNum(et_phoneNumber.getText().toString())) {
				ToastUtil
						.showToast(Activity_ReceiverAddress.this, "请输入合法手机号码");
				return;
			}
	...
	public void onSuccess(ResponseSupport response) {
		...
		if (model.getMessage().equals("操作成功")) {
			ToastUtil.showToast(Activity_ReceiverAddress.this, "操作成功");
			setResult(RESULT_OK);
			finish();
		} else {
			ToastUtil.showToast(Activity_ReceiverAddress.this,
					model.getMessage());
		}
	}
