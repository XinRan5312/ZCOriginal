 ...
 *         2016年6月23日
 */
public class Activity_FaQierDetail extends Activity_Base implements
	...
	protected void initListener() {
		...
		myScrollView.setScrollViewListener(Activity_FaQierDetail.this);
	...
	public void onScrollChanged(ObservableScrollView scrollView, int x, int y,
		...
		} else if (y < (topHeight - title_bar.getHeight())) {
	...
	private void OnClick(View view) {
		...
		case R.id.ll_btn:
			if (ZCApplication.getInstance().isLogin()) {
				if (isFaQi) {
					intent = new Intent(Activity_FaQierDetail.this,
							Activity_MsgSend.class);
					intent.putExtra("name", custName);
					intent.putExtra("custId", custId);
					startActivity(intent);
				} else {
					startActivity(new Intent(Activity_FaQierDetail.this,
							Activity_InvestCerciticyLingTou.class));
				}
			} else {
				new AlertDialog(Activity_FaQierDetail.this).builder()
						.setMsg("您尚未登录，请登录后操作")
...
//				ToastUtil.showToast(Activity_FaQierDetail.this, "暂无数据");
			} else {
				ToastUtil.showToast(Activity_FaQierDetail.this, "已加载全部数据");
			}
			adapter_list.notifyDataSetChanged();
