 ...
 *         2016年8月24日
 */
public class Activity_My_BookedDetail extends Activity_Base implements
		OnItemClickListener {
	...
	protected void initListener() {
		orderNo.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				StringUtils.copyText(Activity_My_BookedDetail.this, orderNo
	...
	private void OnClick(View view) {
		...
		case R.id.btn_ok:
			if (btn_ok.getText().toString().equals(text_cancle)) {
				new AlertDialog(Activity_My_BookedDetail.this).builder()
						.setMsg("取消当前预约将失去优先购买资格，请确认是否要取消预约？")
						.setPositiveButton("立即取消", new OnClickListener() {
			...
			} else {// 先跳转到 订单购买提交页，可选择红包

				Intent intent_pay = new Intent(Activity_My_BookedDetail.this,
						Activity_My_StockBookPay.class);
				intent_pay.putExtra("id", orderId);
				intent_pay.putExtra("prjId", prjId);
				startActivityForResult(intent_pay, AppContants.ACT_REQUEST_CODE);
			}
			break;
		default:
			break;
	...
	public void submitCancelPreOrder(String orderId) {
		...
		request.setSessionId(ZCApplication.getInstance().getUserInfo()
				.getSessionId());
		request.setOrderId(orderId);
		NetWorkRequestManager.sendRequestPost(Activity_My_BookedDetail.this,
				true, request, SubmitCancelOrderResponse.class, this);
	}
	...
	public void commpleteUI(GetMyPreProdOrderByIdResponse model) {
		...
		tv_yuyuejin.setText((investTotal
				* Double.parseDouble(model.getEarnest()) / 100)
				+ "元");
	...
	protected void onActivityResult(int arg0, int arg1, Intent arg2) {
		...
		if (arg1 == RESULT_OK) {
			...
			if (arg0 == AppContants.ACT_REQUEST_CODE) {
				finish();
			}
		}
	...
	public void getAgreement(boolean isShow) {
		Service_Login.getAgreementContent(this, "4", isShow,
				new UserCallBack() {
